        ...
        {requests.length === 0 ? <Loader /> : (
          <div className="contest-editorial-grid">
            {requests.map((request, index) => (
              <button className="contest-editorial-card" key={request.id} onClick={() => setSelected(request)}>
                <span className="card-no">#{String(index + 1).padStart(2, '0')} / {request.status}</span>
                <h3>{request.title}</h3>
                <p className="dim">{request.rules_description}</p>
                <div className="card-meta">
                  <span>{request.requested_type.replaceAll('_', ' ')}</span>
                ...
                <tr key={contest.id}>
                  ...
                  <td>
                    {contest.status === 'PENDING_APPROVAL' && <div className="row" style={{ justifyContent: 'flex-end', gap: 8 }}>
                      <button className="btn danger sm" onClick={() => setRejectTarget(contest)}>Reject</button>
                      <button className="btn primary sm" onClick={() => approve(contest.id)}>Approve</button>
                    </div>}
                    {contest.status === 'ACTIVE' && <button className="btn danger sm" onClick={() => setCancelTarget(contest)}>Cancel</button>}
                  </td>
                </tr>
              ))}
            </tbody>
...
function RequestDecision({ request, onClose, onDone }: { request: ContestTypeRequest; onClose: () => void; onDone: () => void }) {
  ...
  async function decide(decision: 'APPROVED' | 'REJECTED') {
    setBusy(true)
    try {
      await devApi.decideFormatRequest(request.id, decision, note)
  ...
  return (
    <Modal title={request.title} subtitle={`Format proposal · ${request.requested_type.replaceAll('_', ' ')}`} onClose={onClose}
      footer={<>
        <button className="btn ghost" onClick={onClose}>Close</button>
        {isPending && <>
          <button className="btn danger" disabled={busy} onClick={() => decide('REJECTED')}>{busy ? <Spinner /> : 'Reject request'}</button>
          <button className="btn primary" disabled={busy} onClick={() => decide('APPROVED')}>{busy ? <Spinner /> : 'Approve request'}</button>
        </>}
      </>}>
      <div className="notice blue" style={{ marginBottom: 16 }}>
        <b>Requested by {request.requester || 'a member'}</b><br />
        Review the player experience and judge requirements before adding this format to the platform.
      </div>
...
function CancelContestDialog({ contest, onClose, onDone }: { contest: Contest; onClose: () => void; onDone: () => void }) {
  ...
  async function cancel() {
    setBusy(true)
    try {
      await devApi.cancel(contest.id)
      toast('Contest cancelled')
      onDone()
    } catch (error) {
      toast(apiError(error), 'err')
      setBusy(false)
  ...
  return (
    <Modal title={`Cancel ${contest.title}?`} subtitle="Developer action / active contest" onClose={onClose}
...
function RejectContestDialog({ contest, onClose, onDone }: { contest: Contest; onClose: () => void; onDone: () => void }) {
  ...
  async function reject() {
    setBusy(true)
    try {
      await devApi.reject(contest.id)
      toast('Contest rejected')
      onDone()
    } catch (error) {
      toast(apiError(error), 'err')
      setBusy(false)
  ...
  return (
    <Modal title={`Reject ${contest.title}?`} subtitle="Developer action / pending contest" onClose={onClose}
